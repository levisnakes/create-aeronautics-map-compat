...
*Ship radar & map integration for Create: Aeronautics (mod id `aeroradar`).*

**"Where's my airship?" — solved.**

A NeoForge 1.21.1 mod that draws Create: Aeronautics ships **directly on Xaero's
Minimap / World Map and JourneyMap** as native, live-moving map icons, and remembers
each ship's **last-known position**. Client-side is enough; add it to the server for
a shared, persistent, whole-server radar.

## Features

- **Native map icons, not waypoints** — on Xaero's, ships render through the map
  element API (the same path as Xaero's own entity radar); on JourneyMap, through
  its official plugin API as marker overlays. Icons exist only in map views: no
  waypoint-list entries, no in-world beacons, nothing written to the map mods' files.
...
- A map mod to see the icons — any of these, all *optional*:
  - Xaero's Minimap (+ Xaero's World Map on top of it)
  - JourneyMap
...
The build compiles against five jars in `libs/` which are **not** included in this
repository (they are other mods' property — All Rights Reserved / their own
licenses). Copy the Sable + Xaero jars from a modpack that ships Create:
Aeronautics + Xaero's maps (e.g. from your `mods/` folder); the JourneyMap API jar
is extracted from the JourneyMap release jar's `META-INF/jarjar/` folder
...
libs/journeymap-api-neoforge-2.0.0-1.21.1.jar
```
